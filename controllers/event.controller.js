...
const createEvent = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!event) {
            return res.status(400).json({ status: false, message: "Failed to create event" });
        }
        ...
        const invitations = teachers.map(async (teacher) => {
            ...
            const formattedPoints = points && points.length === 3
    ...
    try {
        ...
        await Promise.all(
            teachers.map(async (teacher) => {
                ...
                const formattedPoints = event.points?.length === 3
                ...
                const text = `
                    ...
                    We would like to inform you that there have been some updates to the **"${event.name}"** event. Please find the updated details below:
        ...
        res.status(200).json({ status: true, message: "Event updated successfully and emails sent!", event });
    } catch (error) {
        console.error(error);
        res.status(500).json({ status: false, message: "Internal server error" });
    }
