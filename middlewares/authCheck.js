const { getUser, getClassUser } = require("../helpers/jwt.helper");
...
async function restrictClassLogIn(req, res, next) {
    try {
        // Correct way to access authorization header (case-insensitive)
        // console.log("first",req.headers['authorization'])
        let token = req.headers['authorization']; 
        ...
        // console.log('Authorization token:', token);
        const user = await getClassUser(token);
        if (!user) {
            return res.status(401).json({ status: false, message: "Invalid Login Details" });
        }
        req.user = user;
...
module.exports = { restrictLogIn ,restrictClassLogIn};
